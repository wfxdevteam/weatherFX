...
local tintBase = rgb(1, 1, 1)

local function computeUniqueModifier(s, type)
  return math.lerp(s.currentType == type and 1 or 0, s.upcomingType == type and 1 or 0, s.transition)
end
...
function ReadConditions(dt)
  ...
  cc.cloudsDensity = cc.cloudsDensity +
      (math.lerp(vc.cloudsDensity, vu.cloudsDensity, s.transition) - cc.cloudsDensity) * lagMult
  ...
  tintBase.r = tintBase.r + (math.lerp(vc.tint.r, vu.tint.r, s.transition) - tintBase.r) * lagMult
  tintBase.g = tintBase.g + (math.lerp(vc.tint.g, vu.tint.g, s.transition) - tintBase.g) * lagMult
  tintBase.b = tintBase.b + (math.lerp(vc.tint.b, vu.tint.b, s.transition) - tintBase.b) * lagMult

  if ScriptSettings.LINEAR_COLOR_SPACE.BLUE_NIGHTS == 0 then
    cc.tint = tintBase
  else
    local blueNight = math.max(ctx.nightK * 2 - 1, 0)
    cc.tint.r = tintBase.r * (1 - blueNight * 0.5)
    cc.tint.g = tintBase.g
    cc.tint.b = tintBase.b * (1 + blueNight)

    if ScriptSettings.LINEAR_COLOR_SPACE.BLUE_NIGHTS ~= 1 then
      ApplyExtraVibe(cc.tint)
    end
  end

  local ash = computeUniqueModifier(s, ac.WeatherType.Smoke) * 0.1
  local newSnow = math.max(0,
    math.lerp(vc.snow, vu.snow, s.transition) - math.lerpInvSat(Sim.ambientTemperature, 0, 5) * 0.05)
  cc.snow = cc.snow + (newSnow - cc.snow) * lagMult
  ac.setWeatherParticles('ash', ash, 0)
  ac.setWeatherParticles('snow', cc.snow, cc.snow ^ 2)
  ApplyTornado(computeUniqueModifier(s, ac.WeatherType.Tornado))
