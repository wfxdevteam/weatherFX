...
local MEASURE_PERFORMANCE = const(false)
...
if not ac.getSim().isPostProcessingActive then
  -- LCS is not available without post-processing
...
if ScriptSettings.LINEAR_COLOR_SPACE.DEV_MODE then
  ui.addSettings({
    icon = ui.Icons.WeatherFewClouds,
    name = 'WeatherFX style',
    onOpen = function()
...
local function runFrame(task)
  local r = runFrames[task]
  if not r then
    r = {
      name1 = 'task: %s, μs' % task,
      name2 = 'task: %s, KB' % task,
      avg = 0,
      start = function(s)
        -- collectgarbage("collect")
        -- collectgarbage("collect")
        s.time = os.preciseClock()
      end,
      done = function(s)
        local taken = os.preciseClock() - s.time
        s.avg = math.lerp(s.avg, taken, 0.02)
        -- local garbage = collectgarbage('collect')
        ac.debug(s.name1, s.avg * 1e6)
        -- ac.debug(s.name2, garbage / 1024)
      end
    }
    runFrames[task] = r
  end
...
function script.update(dt)
  ...
  if MEASURE_PERFORMANCE then task = runFrame('base') end
  ruBase:update(gameDT, forceUpdateShading)
  if MEASURE_PERFORMANCE then task:done() end

  if MEASURE_PERFORMANCE then task = runFrame('cloud mat') end
  ruCloudMaterials:update(CloudsDT, forceUpdateShading)
  if MEASURE_PERFORMANCE then task:done() end

  -- Increasing refresh rate for faster moving clouds
  if MEASURE_PERFORMANCE then task = runFrame('clouds') end
  ...
  ruClouds:update(CloudsDT, forceUpdate or Sim.cameraPositionDelta:lengthSquared() > 15)
  if MEASURE_PERFORMANCE then task:done() end

  -- Fake exposure aka eye adaptation needs to update each frame, with speed independant
  -- from pause, slow motion and what not
  if MEASURE_PERFORMANCE then task = runFrame('exposure') end
  ...
  if MEASURE_PERFORMANCE then task = runFrame('end') end
